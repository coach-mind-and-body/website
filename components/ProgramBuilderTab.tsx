  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-bold text-2xl" style={{ fontFamily: "'Cormorant Garamond', serif", color: "oklch(0.20 0.015 50)" }}>Reclaim Program Builder</h2>
        <div className="flex gap-2">
          <Button 
            ...
            style={{ 
              ...
              color: view === "modules" ? "oklch(0.96 0.025 50)" : "oklch(0.72 0.12 75)" 
            }}
          >
            Modules
          </Button>
          <Button 
            ...
            style={{ 
              ...
              borderColor: "oklch(0.72 0.12 75)",
              color: view === "submissions" ? "oklch(0.96 0.025 50)" : "oklch(0.72 0.12 75)" 
            }}
          >
            Submissions
          </Button>
          ...
          {!editingModule ? (
            <>
              ...
              <div className="space-y-4">
                {(modules || []).map(mod => (
                  <div key={mod.id} className="p-5 rounded-xl flex items-center justify-between" style={{ background: "oklch(1 0 0)" }}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm" style={{ background: "oklch(0.985 0.008 80)", color: "oklch(0.72 0.12 75)" }}>
                        {mod.order}
                      </div>
                      <div>
                        <h3 className="font-semibold" style={{ color: "oklch(0.20 0.015 50)" }}>{mod.title}</h3>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => { if(confirm("Delete this module?")) deleteModule.mutate({ id: mod.id }) }} style={{ color: "oklch(0.52 0.015 50)" }}>
                        <Trash2 size={16} />
                      </Button>
                    </div>
                  </div>
                ))}
                {modules?.length === 0 && <p className="text-sm" style={{ color: "oklch(0.52 0.015 50)" }}>No modules created yet.</p>}
              </div>
            </>
          ) : (
            <div className="p-6 rounded-xl space-y-4" style={{ background: "oklch(1 0 0)" }}>
              <h3 className="font-bold text-xl mb-4" style={{ color: "oklch(0.20 0.015 50)" }}>{editingModule.id ? "Edit Module" : "New Module"}</h3>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label style={{ color: "oklch(0.42 0.015 50)" }}>Title</Label>
                  <Input 
                    value={editingModule.title} 
                    onChange={e => setEditingModule({...editingModule, title: e.target.value})} 
                    style={{ background: "oklch(0.96 0.025 50)", borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.20 0.015 50)" }}
                  />
                </div>
                <div>
                  <Label style={{ color: "oklch(0.42 0.015 50)" }}>Order (1-6)</Label>
                  <Input 
                    type="number" 
              ...
              <div>
                <Label style={{ color: "oklch(0.42 0.015 50)" }}>Description (Short intro)</Label>
                <Textarea 
              ...
              <div>
                <Label style={{ color: "oklch(0.42 0.015 50)" }}>Video URL (YouTube/Vimeo embed URL)</Label>
                <Input 
              ...
              <div>
                <Label style={{ color: "oklch(0.42 0.015 50)" }}>Content (Rich text/HTML)</Label>
                <div className="mt-2" style={{ border: "1px solid oklch(0.90 0.015 80)", borderRadius: "0.5rem", overflow: "hidden" }}>
                  <RichTextEditor 
              ...
              <div>
                <Label style={{ color: "oklch(0.42 0.015 50)" }}>PDF Download URL (Optional)</Label>
                <div className="flex items-center gap-2 mt-2">
                  <Input 
                    value={editingModule.pdfUrl || ""} 
                    onChange={e => setEditingModule({...editingModule, pdfUrl: e.target.value})} 
                    style={{ background: "oklch(0.96 0.025 50)", borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.20 0.015 50)", flex: 1 }}
                    placeholder="https://..."
                  />
                  <div className="flex-shrink-0">
                    ...
                    <label 
                      ...
                      style={{ background: "oklch(0.985 0.008 80)", color: "oklch(0.20 0.015 50)", border: "1px solid oklch(0.90 0.015 80)" }}
                    >
              ...
              <div className="flex items-center gap-2 mt-4">
                ...
                />
                <Label htmlFor="isPublished" style={{ color: "oklch(0.42 0.015 50)" }}>Publish this module (Clients can see it when unlocked)</Label>
              </div>

              {editingModule.id && (
                <div className="mt-8 pt-6 border-t" style={{ borderColor: "oklch(0.90 0.015 80)" }}>
                  <ModuleAssignmentsEditor moduleId={editingModule.id} />
                </div>
              )}

              <div className="flex justify-end gap-3 pt-4 mt-4 border-t" style={{ borderColor: "oklch(0.90 0.015 80)" }}>
                <Button variant="outline" onClick={() => setEditingModule(null)} style={{ borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.42 0.015 50)" }}>Cancel</Button>
                <Button 
                  onClick={() => {
                    if (editingModule.id) {
      ...
      {view === "submissions" && (
        <div className="space-y-4">
          {(submissions || []).map((item: any) => (
            <div key={item.submission.id} className="p-5 rounded-xl" style={{ background: "oklch(1 0 0)" }}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="font-bold" style={{ color: "oklch(0.20 0.015 50)" }}>Module {item.module.order}: {item.module.title}</h3>
                  <p className="text-xs mt-1" style={{ color: "oklch(0.52 0.015 50)" }}>
              ...
              <div>
                <Label style={{ color: "oklch(0.42 0.015 50)" }}>Your Feedback</Label>
                <Textarea 
          ...
          {submissions?.length === 0 && <p className="text-sm" style={{ color: "oklch(0.52 0.015 50)" }}>No assignment submissions yet.</p>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-lg" style={{ color: "oklch(0.20 0.015 50)" }}>Tasks & Assignments</h3>
        <Button 
          size="sm" 
          onClick={() => setAdding(!adding)}
          style={{ background: "oklch(0.985 0.008 80)", color: "oklch(0.20 0.015 50)" }}
        >
      ...
      {adding && (
        <div className="p-4 rounded-lg mb-4" style={{ background: "oklch(0.96 0.025 50)", border: "1px solid oklch(0.90 0.015 80)" }}>
          <Label style={{ color: "oklch(0.42 0.015 50)" }}>Task Prompt / Question</Label>
          <Textarea 
            value={newQuestion}
            onChange={(e) => setNewQuestion(e.target.value)}
            placeholder="e.g. What are your biggest triggers for food noise?"
            className="mt-2 mb-3"
            style={{ background: "oklch(0.985 0.008 80)", borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.20 0.015 50)" }}
          />
          <div className="flex justify-end gap-2">
            <Button size="sm" variant="outline" onClick={() => setAdding(false)} style={{ borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.42 0.015 50)" }}>Cancel</Button>
            <Button 
              size="sm" 
      ...
      <div className="space-y-3">
        {(assignments || []).map((assignment, index) => (
          <div key={assignment.id} className="p-4 rounded-lg flex flex-col gap-3" style={{ background: "oklch(0.96 0.025 50)", border: "1px solid oklch(0.90 0.015 80)" }}>
            {editingId === assignment.id ? (
              <>
                ...
                />
                <div className="flex justify-end gap-2">
                  <Button size="sm" variant="outline" onClick={() => setEditingId(null)} style={{ borderColor: "oklch(0.90 0.015 80)", color: "oklch(0.42 0.015 50)" }}>Cancel</Button>
                  <Button 
                    size="sm" 
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5" style={{ background: "oklch(0.985 0.008 80)", color: "oklch(0.72 0.12 75)" }}>
                    {index + 1}
                  </div>
                  <p className="text-sm" style={{ color: "oklch(0.20 0.015 50)" }}>{assignment.question}</p>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => { if(confirm("Delete task?")) deleteAssignment.mutate({ id: assignment.id }); }} style={{ color: "oklch(0.52 0.015 50)" }}>
                    <Trash2 size={14} />
                  </Button>
                </div>
              </div>
        ...
        {assignments?.length === 0 && !adding && <p className="text-sm" style={{ color: "oklch(0.52 0.015 50)" }}>No tasks for this module yet.</p>}
      </div>
    </div>
  );
}
