...
export async function POST(req: Request) {
  ...
  try {
      ...
      // Respond to Stripe *immediately* (200). This prevents retries and "trouble sending" alerts.
      // All business logic (DB, emails, Meta CAPI) runs asynchronously so slow operations don't
      // cause timeouts or 5xx responses to Stripe.
      const response = NextResponse.json({ received: true });

      processStripeEvent(event).catch((err) => {
        console.error("[Stripe Webhook] Async processing failed (webhook already ack'd):", err);
      });
